	...
	if( isset( $p['action'] ) )
	{
		...
		else
		{
			...
			if( isset($_SESSION['playerid']) )
			{
				...
				else if( $p['action'] == 'getdailyprogress' )	$rc = Leaderboard_GetDailyProgress( $_SESSION['playerid'] );
				else if( $p['action'] == 'getleaderboard2' )
				{
					$tier = isset($_POST['tier']) ? $_POST['tier'] : 'daily';
					$scope = isset($_POST['scope']) ? $_POST['scope'] : 'friends';
					$rc = Leaderboard_GetBoard( $_SESSION['playerid'], $tier, $scope );
				}
